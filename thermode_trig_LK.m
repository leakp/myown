function [log] = thermode_trig_LK(temp_list)
...
degrees_sec = 5; % search rate (°C/sec)
...
config_io; % load driver

outp(portAdd,0); % set currents of all pins to zero
...
for trial = 1:size(temp_corr,2)
    ...
    lower_temp;% send temperature (time to rise = wait_sec)
    pause(trial_dur-wait_sec); % duration until offset -> start back to baseline
    higher_temp; % go back to baseline
...
% ------------------------------------------------------------------------
    function trig_manual
        ...
        outp64(portAdd,trig_go); % send manual trigger
        pause(.001); % wait for xy sec
        outp64(portAdd,0);
    end

    function higher_temp
        outp64(portAdd,trig_high); % send higher
        pause(wait_sec); % wait for xy sec
        outp64(portAdd,0);
    end

    function lower_temp
        outp64(portAdd,trig_low); % send lower
        pause(wait_sec); % wait for xy sec
        outp64(portAdd,0);
    end
